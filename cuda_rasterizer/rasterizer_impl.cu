	...
	dim3 grid)
{
	...
	if (radii[idx] > 0)
	{
        ...
        getRect(points_xy[idx], ad_radii[idx], rect_min, rect_max, grid);
...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P)
{
    ...
    obtain(chunk, geom.ad_radii, P, 128);
	...
	bool debug)
{
	...
	const float inv_alpha_low = 255.0f;
...
// Forward rendering procedure for differentiable rasterization
// of Gaussians. - Pure version
int CudaRasterizer::Rasterizer::forwardPure(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	bool debug)
{
	const float focal_y = height / (2.0f * tan_fovy);
	const float focal_x = width / (2.0f * tan_fovx);
	const float inv_alpha_low = 100.0f;
