	...
	bool prefiltered)
{
    ...
    float my_radius = ceil(3.f * sqrt(lambda));
    ...
    // Use AABB instead of 99% confidence interval
    getRect(point_image, my_adr, rect_min, rect_max, grid);
...
// Main rasterization method. Collaboratively works on one tile per
// block, each thread treats one pixel. Alternates between fetching
// and rasterizing data. - Pure version
template <uint32_t CHANNELS>
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y)
renderPureCUDA(
	const uint2* __restrict__ ranges,
...
void FORWARD::renderPure(
	...
	float* out_color)
{
	renderPureCUDA<NUM_CHANNELS> << <grid, block >> > (
		ranges,
		point_list,
		W, H,
		means2D,
