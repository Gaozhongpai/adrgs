...
__forceinline__ __device__ uint2 culling_and_blending(
    ...
    uint2* channels_smem,
    const float* log_opacity_smem,
    const float* opacity_smem,
    const float* beta_smem,
    bool has_beta,
    half &T, int gs_index, int thread_id, uint2 RGBD
)
{
    const float ln2_twice = 2.0f * TCGS_UTIL::LN2;
    const float min_alpha = 1.0f / 255.0f;
    ...
    for(int k = 0; k < 8; ++k)
    {
        ...
        if(!has_beta)
        {
            if(__hgt(exponents.x, __float2half_rn(-7.995f)) && __hlt(exponents.x, __float2half_rn(0.0f)))
            {
                ...
                uint alpha2 = half22uint(__halves2half2(alpha, alpha));
        ...
        int base_idx = gs_index | (k << 1);

        float opacity0 = opacity_smem[base_idx];
        if(opacity0 > 0.0f)
        {
            float exponent0 = __half2float(exponents.x);
            float sigma0 = (log_opacity_smem[base_idx] - exponent0) * ln2_twice;
            if(sigma0 >= 0.0f && sigma0 < 1.0f)
            {
                float base0 = 1.0f - sigma0;
                float weight0 = opacity0 * __powf(base0, beta_smem[base_idx]);
                weight0 = fminf(weight0, 0.99f);
                if(weight0 > min_alpha)
                {
                    half weight0_h = __float2half_rn(weight0);
                    half alpha0 = __hmul(T, weight0_h);
                    if(__hgt(alpha0, __float2half_rn(0.0f)))
                    {
                        T = __hsub(T, alpha0);
                        uint2 channel0 = channels_smem[base_idx];
                        uint alpha2_0 = half22uint(__halves2half2(alpha0, alpha0));
                        RGBD.x = fast_fma_rn_ftz_f16x2(channel0.x, alpha2_0, RGBD.x);
                        RGBD.y = fast_fma_rn_ftz_f16x2(channel0.y, alpha2_0, RGBD.y);
                    }
                }
            }
        }
        ...
        int idx1 = base_idx | 1;
        float opacity1 = opacity_smem[idx1];
        if(opacity1 > 0.0f)
        {
            float exponent1 = __half2float(exponents.y);
            float sigma1 = (log_opacity_smem[idx1] - exponent1) * ln2_twice;
            if(sigma1 >= 0.0f && sigma1 < 1.0f)
            {
                float base1 = 1.0f - sigma1;
                float weight1 = opacity1 * __powf(base1, beta_smem[idx1]);
                weight1 = fminf(weight1, 0.99f);
                if(weight1 > min_alpha)
                {
                    half weight1_h = __float2half_rn(weight1);
                    half alpha1 = __hmul(T, weight1_h);
                    if(__hgt(alpha1, __float2half_rn(0.0f)))
                    {
                        T = __hsub(T, alpha1);
                        uint2 channel1 = channels_smem[idx1];
                        uint alpha2_1 = half22uint(__halves2half2(alpha1, alpha1));
                        RGBD.x = fast_fma_rn_ftz_f16x2(channel1.x, alpha2_1, RGBD.x);
                        RGBD.y = fast_fma_rn_ftz_f16x2(channel1.y, alpha2_1, RGBD.y);
                    }
                }
            }
        }
...
) 
{
    ...
    __shared__ float log_opacity_smem[BLOCK_SIZE_TCGS];
    __shared__ float opacity_smem[BLOCK_SIZE_TCGS];
    __shared__ float beta_smem[BLOCK_SIZE_TCGS];
    ...
    for(int i = 0; i < rounds; ++i, toDo -= BLOCK_SIZE_TCGS)
    {
        ...
        if(range.x + progress < range.y)
        {
            ...
            log_opacity_smem[thread_id] = conics.w;
            opacity_smem[thread_id] = exp2f(conics.w);
            beta_smem[thread_id] = has_beta ? betas[coll_id] : 0.0f;
        }
        else
        {
            log_opacity_smem[thread_id] = -1e9f;
            opacity_smem[thread_id] = 0.0f;
            beta_smem[thread_id] = 0.0f;
        }
...
)
{
    ...
    renderCUDA_TCGS<< <grid, block>> >(
        ranges, point_list,
        width, height,
        means2D, feature_encoded, betas, conic_opacity,
        final_T, n_contrib,
